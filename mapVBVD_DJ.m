function twix_obj = mapVBVD_DJ(filename,varargin)
...
% DJ 2022-4-22: add option to combine IMASCAN and REFSCAN into a single k-space
arg.CombineImaRef   = false;
% DJ 2022-4-22: end of change
...
while k <= numel(varargin)
    ...
    switch lower(varargin{k})
        ...
        % DJ 2022-4-22: add option to combine IMASCAN and REFSCAN into a single k-space
        case 'imaref' % combine IMASCAN and REFSCAN into a single k-space
            if numel(varargin) > k && ~ischar(varargin{k+1})
                arg.CombineImaRef = logical(varargin{k+1});
                k = k+2;
            else
                arg.CombineImaRef = true;
                k = k+1;
            end
        % DJ 2022-4-22: end of change
        otherwise
