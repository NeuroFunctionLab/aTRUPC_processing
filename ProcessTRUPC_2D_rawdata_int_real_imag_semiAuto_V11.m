...
figure,imshow(C1);title('Static vessel ROI-Automatic');
...
figure,imshow(C);title('Static tissue ROI-Automatic');
...
bBatch = 1;
for iSet = 1:nSet-1
    [CD_correct(:, iSet), dphi_correct(:, iSet), phi_fit{iSet}, cc_fit{iSet}, mask_static_refined] = CDPhaCor_Iter_2D_V2...
...
saveas(gcf, [results_folder, filesep, 'Refst_ROI_Auto.fig']);
...
branchlimit = 20;
[skelb_SSS,skel_SSS, coor_AP, coor_disp] = sortedskel(BW_SSS,branchlimit);
...
branchlimit = 5;
[skelb_SS,skel_SS, coor_AP, coor_disp] = sortedskel(BW_SS,branchlimit);
...
plot(xcol,yrow,'r.');
hold off;
[coor_AP,coor_disp] = sortedskel2(skefit);%--extended and sorted skeleton
